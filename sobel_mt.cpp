...
void *runSobelMT(void *ptr)
{
  ...
  while (1) {
    printf("Thread %d is starting frame %d\n", myID, i);
    ...
    if (myID == thread0_id) {
      ...
      pthread_barrier_wait(&thread0First);
    } else {
      pthread_barrier_wait(&thread0First);
    }
    ...
    printf("Starting gray parallel section %d\n", myID);
    printf("IMG_WIDTH, IMG_HEIGHT = %d, %d\n", IMG_WIDTH, IMG_HEIGHT);
    // LAB 2, PART 2: Start parallel section

    // Mat src1 = src(Rect(0, 0, IMG_WIDTH/2+1, IMG_HEIGHT)).clone();
    // Mat src2 = src(Rect(IMG_WIDTH/2-1, 0, IMG_WIDTH/2+1, IMG_HEIGHT)).clone();
    // Mat img_gray1 = img_gray(Rect(0, 0, IMG_WIDTH/2+1, IMG_HEIGHT));
    // Mat img_gray2 = img_gray(Rect(IMG_WIDTH/2-1, 0, IMG_WIDTH/2+1, IMG_HEIGHT));
    // Mat img_sobel1 = img_sobel(Rect(0, 0, IMG_WIDTH/2+1, IMG_HEIGHT));
    // Mat img_sobel2 = img_sobel(Rect(IMG_WIDTH/2-1, 0, IMG_WIDTH/2+1, IMG_HEIGHT));
    ...
    Mat img_sobel2 = img_sobel(Rect(0, IMG_HEIGHT/2-1, IMG_WIDTH, IMG_HEIGHT/2+1));


    
    // use myID to distinguish between the two threads
    pc_start(&perf_counters);
    if (myID == thread0_id) {
      grayScale(src1, img_gray1);
    } else {
      grayScale(src2, img_gray2);
    }
    pthread_barrier_wait(&endGrayConv);
    
    pc_stop(&perf_counters);

    printf("Starting sobel section %d\n", myID);
    ...
    printf("Ending sobel section %d\n", myID);
    // hang after Ending sobel section
    ...
    if (myID == thread0_id) {
      ...
      // Press q to exit
      // only one thread catches the keypress
      char c = cvWaitKey(10);
      if (c == 'q' || i >= opts.numFrames) {
        quit = true;
        printf("thread 0 is quitting\n");
        pthread_barrier_wait(&quitSobel);
        printf("thread 0 is done quitting\n");
        break;
      }
    }

    printf("thread 1 is quitting\n");
    pthread_barrier_wait(&quitSobel);
    printf("thread 1 is done quitting, %d\n", quit);
  ...
  pthread_barrier_wait(&endSobel);
  printf("thread %d is done after endSobel\n", myID);
  return NULL;
}
