...
pthread_barrier_t endGrayConv;
pthread_barrier_t endSobelConv;
pthread_barrier_t thread0First;
pthread_barrier_t quitSobel;
...
int mainMultiThread()
{
  ...
  pthread_barrier_init(&endGrayConv, NULL, 2);
  pthread_barrier_init(&endSobelConv, NULL, 2);
  pthread_barrier_init(&thread0First, NULL, 2);
  pthread_barrier_init(&quitSobel, NULL, 2);
  ...
  pthread_barrier_destroy(&endSobelConv);
  pthread_barrier_destroy(&endGrayConv);
  pthread_barrier_destroy(&thread0First);
  pthread_barrier_destroy(&quitSobel);
